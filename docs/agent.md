...
- Topology should keep parsed zone/surface/window geometry data in the backend report and render from that shared structure in 3D and Plan views. Level is shared across 3D, Plan, and Network: All shows the full model and a specific level applies the former story filter. 3D and Plan share Zones/Surfaces/Openings visibility, while Network exposes only Metric and Layout. A selected zone, boundary, or other topology object should emphasize its one-hop related objects and strongly fade unrelated objects; selected-object details should show metrics plus related objects instead of duplicating IDF fields, and Sync locate controls automatic jumps to matching input objects.
