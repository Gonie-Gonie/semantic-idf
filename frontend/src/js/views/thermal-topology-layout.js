...
export function createThermalTopologyLayoutModel(geometry, options = {}) {
  ...
  const expanded = options.expandExternalTargets ? expandExternalTargets(base) : base;
  const scoped = applyThermalTopologyScope(expanded, options);
  return {
    ...scoped,
...
function isExternalNode(node) {
  ...
  return node.kind !== "zone" && node.kind !== "space";
}
