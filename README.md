...
- Topology parses detailed zones, walls, roofs, floors, and fenestration into 3D, Plan, and Network views. A shared Level selector shows all levels or isolates one level, 3D and Plan share Zones/Surfaces/Openings visibility, and Network exposes only Metric and Layout controls. Selecting an object emphasizes its one-hop connections and strongly fades unrelated objects; Sync locate jumps to the matching input object.
