...
export function captureSimulationEnergyWorkspaceContext() {
  ...
  return { ...primary, energyDrawer: { ...drawer },
    energyChartFrequency: state.simulationEnergyChartFrequency === "hourly" ? "hourly" : "monthly" };
}
...
export function restoreSimulationEnergyWorkspaceContext(snapshot = {}) {
  ...
  state.simulationEnergyChartFrequency = snapshot.energyChartFrequency === "hourly" ? "hourly" : "monthly";
...
export function initializeSimulationControls() {
  ...
  elements.simulationHVACLoopResults?.addEventListener("click", handleSimulationHVACResultsInput);
  ...
  elements.simulationHVACLoopResults?.addEventListener("keydown", (event) => {
    if (["Enter", " "].includes(event.key) && event.target.matches("[data-hvac-inspect-node], [data-hvac-inspect-component]")) {
...
function simulationNavigationDestination(selection = {}, context = {}) {
  ...
  if (servicePathTarget) {
    ...
    if (item) {
      return {
        resultView: "energy",
        energyScopeKind: item.navigationScopeKind || state.simulationEnergyScopeKind,
        energyZoneName: item.navigationZoneName || state.simulationEnergyZoneName,
  ...
  if (entity?.kind === "hvac-loop") {
    ...
    if (loop) {
      return {
        ...
        hvacInspectionLoop: hvacInspectionLoopKey(loop),
...
function simulationEnergyItemForServicePath(pathID = "") {
  const explanation = state.simulationResult?.purposeResults?.energyExplanation || {};
  const views = [state, ...energyPathZoneNames(explanation).map((zoneName) => ({
    ...state, simulationEnergyScopeKind: "zone", simulationEnergyZoneName: zoneName,
  }))];
  for (const viewState of views) {
    const graph = energyPathGraphForState(explanation, viewState);
    const linked = (graph.links || []).filter((link) => (link.relatedPathIds || []).includes(pathID));
    const ids = new Set(linked.flatMap((link) => [link.fromId, link.toId]));
    const item = (graph.nodes || []).find((node) => (node.relatedPathIds || []).includes(pathID))
      || (graph.nodes || []).find((node) => ["load", "end_use"].includes(node.level) && ids.has(node.id));
    if (item) return { ...item, navigationScopeKind: viewState.simulationEnergyScopeKind, navigationZoneName: viewState.simulationEnergyZoneName };
  }
  return null;
}
...
function simulationZoneNavigationDestination(zoneName = "", contextKind = "") {
  ...
  const explanation = state.simulationResult?.purposeResults?.energyExplanation || {};
  const energyZone = energyPathZoneNames(explanation).find((name) => simulationZoneKey(name) === zoneKey);
  const graph = energyPathGraphForState(explanation, { ...state, simulationEnergyScopeKind: "zone", simulationEnergyZoneName: energyZone || zoneName });
  const energyItem = (graph.nodes || []).find((item) => simulationZoneKey(item.zoneName || "") === zoneKey) || graph.nodes?.[0];
  ...
  if (energyZone) {
    return {
      resultView: "energy",
      energyScopeKind: "zone",
      energyZoneName: energyZone,
      energySelection: energyItem?.id || "",
      panelTargetID: energyItem ? `simulation:energy:node:${state.simulationEnergyPeriod || "annual"}:${energyItem.id}` : "",
    };
  }
  if (heatZone) {
    return {
...
function applySimulationNavigationDestination(destination = {}) {
  ...
  if (destination.hvacInspectionLoop) simulationHVACInspectionContext().context.selectedLoop = destination.hvacInspectionLoop;
  for (const [key, value] of Object.entries({
    simulationEnergyScopeKind: destination.energyScopeKind,
    simulationEnergyZoneName: destination.energyZoneName,
...
export function captureSimulationNavigationContext(context) {
  ...
  return {
    ...
    seriesPanels: simulationSeriesPanels.map((panel) => ({ ...panel, seriesIDs: [...panel.seriesIDs] })),
...
export async function restoreSimulationNavigationContext(snapshot = {}, context) {
  ...
  } else if (snapshot.selectedSeries) {
    simulationSeriesPanels = [{ id: simulationSeriesPanelSequence++, seriesIDs: [snapshot.selectedSeries], start: Number(snapshot.seriesRangeStart) || 0, end: snapshot.seriesRangeEnd ?? -1 }];
  }
...
function preferredSimulationSemanticOccurrence(selection = {}, context = {}) {
  ...
  const selectedPaths = simulationSelectedEnergyPathNode()?.relatedPathIds || [];
  const pathToken = selectedPaths.length === 1 ? simulationSemanticToken(selectedPaths[0]) : "";
  const anchor = selection.sourceAnchor || {};
...
function simulationPreferredOccurrenceContexts() {
  switch (state.simulationActiveResultView) {
    ...
    case "energy":
      return state.simulationEnergyDetailsOpen
...
function renderSimulationEmpty({ controlsReady = false, blockingIssue: preparedBlockingIssue } = {}) {
  ...
  setSimulationSeriesGroupUnavailable();
  elements.simulationChart.innerHTML = `<div class="empty">${t("simulation.noGraph", {}, "Variables will appear after a run with numeric output.")}</div>`;
  toggleSimulationResultSections();
...
function prepareSimulationEnergyScene(result) {
  state.simulationEnergyService = "all";
  ...
  return simulationEnergySceneSlot.acquire(simulationEnergySceneContext(result), () => {
    ...
    const path = prepareEnergyPathScene(explanation, state, { graph, allServiceGraph: graph, display });
    return { result, path };
  });
}
...
function updateSimulationEnergyPanel(kind) {
  ...
  const refreshInspector = kind === "chart" || (scene && context.some((value, index) => value !== scene.inspectorContext?.[index]));
...
export function renderSimulationEnergyDashboard(result) {
  ...
  if (useEnergyPathV2) {
    elements.simulationEnergyDashboard.innerHTML = renderEnergyPathView(explanation, state, simulationEnergySceneOptions(scene));
...
export function simulationEnergyInspectorActions(node = {}, sourceDetails = [], viewState = state, result = state.simulationResult, preparedGraph) {
  ...
  const candidates = resolveEnergyPathSeriesCandidates(node, sources, result?.series || []);
  const series = candidates.filter((candidate) => candidate.status === "exact" && energyPathSeriesPeriodRange(candidate.series, period))
    .map((candidate) => ({
      id: candidate.seriesId,
      sourceId: candidate.source.id,
      label: simulationSeriesDisplayColumn(candidate.series),
  ...
  const relatedPathIDs = new Set([
    ...(node.relatedPathIds || []),
    ...(graph.links || []).filter((link) => ids.has(link.fromId) || ids.has(link.toId)).flatMap((link) => link.relatedPathIds || []),
  ]);
  const hvacPaths = ["load", "end_use"].includes(node.level)
    ? simulationHVACServicePaths().filter((path) => relatedPathIDs.has(path.id))
      .map((path) => ({ id: path.id, label: simulationEnergyServicePathFocusLabel(path) }))
    : [];
  const pathLabelCounts = new Map();
  for (const path of hvacPaths) pathLabelCounts.set(path.label, (pathLabelCounts.get(path.label) || 0) + 1);
  for (const path of hvacPaths) if (pathLabelCounts.get(path.label) > 1) path.label = `${path.label} · ${path.id}`;
  return {
    series, hvacPaths,
    seriesUnavailableReason: candidates.some((candidate) => candidate.status === "exact")
      ? period !== "annual"
        ? t("simulation.energyPathSeriesPeriodUnavailable", {}, "No reliable reported Series data is available for the selected month.")
        : t("simulation.energyPathSeriesDataUnavailable", {}, "The matching series has no usable reported values for this period.")
      : candidates.length
        ? t("simulation.energyPathSeriesAmbiguous", {}, "More than one reported series has this identity; choose a variable in Series.")
    ...
    .map((path) => {
      ...
      return `
        <tr>
          ...
          <td>${renderSimulationEnergyConnectedSystems(path)}</td>
          <td>${renderSimulationEnergySupportingAssets(path)}</td>
...
function simulationServicePathLoopRefs(path = {}) {
  return [
    path.plantLoop?.name ? { type: path.plantLoop.type || "PlantLoop", name: path.plantLoop.name, label: `PlantLoop ${path.plantLoop.name}` } : null,
    path.airLoop?.name ? { type: path.airLoop.type || "AirLoopHVAC", name: path.airLoop.name, label: `AirLoopHVAC ${path.airLoop.name}` } : null,
    path.condenserLoop?.name ? { type: path.condenserLoop.type || "CondenserLoop", name: path.condenserLoop.name, label: `CondenserLoop ${path.condenserLoop.name}` } : null,
  ].filter(Boolean);
}

function simulationServicePathOtherSystems(path = {}) {
  return [
    path.refrigerantSystem?.name ? `${path.refrigerantSystem.type || "Refrigerant"} ${path.refrigerantSystem.name}` : "",
    path.sourceSystem?.name ? `${path.sourceSystem.type || "Source"} ${path.sourceSystem.name}` : "",
  ].filter(Boolean);
...
function simulationServicePathSupportingAssetRefs(path = {}) {
  ...
  return (path.supportingCouplingIds || [])
    .map((id) => {
      ...
      return {
        id,
        label: coupling.object?.displayName || coupling.object?.objectName || coupling.role || id,
      };
    })
...
function energyEndUseLabel(endUse = "") {
  const labels = {
    ...
    exterior_equipment: t("simulation.energyEndUseExteriorEquipment", {}, "Exterior equipment"),
    ...
    humidification: t("simulation.energyEndUseHumidification", {}, "Humidification"),
...
function simulationHVACInspectionContext(result = state.simulationResult) {
  const loops = result?.purposeResults?.hvacLoops || [];
  const runID = result?.runId || result?.inputPath || result?.filename || "result";
  if (!state.simulationHVACInspection || state.simulationHVACInspection.runID !== runID) {
    state.simulationHVACInspection = { runID, selectedLoop: "", loops: {} };
  }
  const context = state.simulationHVACInspection;
  context.loops ||= {};
  const loop = loops.find((item) => hvacInspectionLoopKey(item) === context.selectedLoop)
    || loops.find((item) => item.series?.length || item.components?.some((component) => component.series?.length)) || loops[0];
  if (!loop) return { loops, context };
...
function renderSimulationHVACLoops(result) {
  const { loops, loop, ui, context } = simulationHVACInspectionContext(result);
  if (!loop) {
    renderSimulationHVACLoopEmpty(t("simulation.noHVACLoopResult", {}, "Run HVAC Loop Check to inspect node state series."));
    return;
  }
  ...
  elements.simulationHVACLoopResults.innerHTML = `
    ...
    <section class="simulation-hvac-loop-result" ${simulationHVACLoopSemanticAttributes(loop)}>
      ${renderHVACInspection(loop, ui)}
    </section>`;
...
function simulationSeriesLookup() {
  ...
  for (const item of series) {
    ...
    // Keep duplicate identities ambiguous instead of silently retaining first.
    if (id) byID.set(id, byID.has(id) ? null : item);
    const legacyID = `${item.file || ""}::${item.column || ""}`;
    legacyIDs.set(legacyID, legacyIDs.has(legacyID) ? null : item);
  ...
  // Old saved selections can migrate only when their alias is unambiguous.
  for (const [id, item] of legacyIDs) if (!byID.has(id)) byID.set(id, item);
...
export function handleSimulationSeriesInspectClick(event) {
  ...
  if (energyZoneJump) {
    const explanation = state.simulationResult?.purposeResults?.energyExplanation || {};
    const zoneName = energyZoneJump.dataset.simulationEnergyZoneJump || "";
    if (!energyPathZoneNames(explanation).includes(zoneName)) return;
    state.simulationEnergyScopeKind = "zone";
    state.simulationEnergyZoneName = zoneName;
    state.simulationEnergySelection = "";
    renderSimulationEnergyDashboard(state.simulationResult);
    return;
  }
...
function simulationHVACLoopForRef(loopType = "", loopName = "") {
  ...
  const wantedType = simulationHVACGraphName(loopType);
  return (state.report?.hvac?.loops || []).find((loop) => {
    if (simulationHVACGraphName(loop.name) !== wantedName) {
      return false;
    }
    return !wantedType || simulationHVACGraphName(loop.type) === wantedType;
  }) || (state.report?.hvac?.loops || []).find((loop) => simulationHVACGraphName(loop.name) === wantedName) || null;
}

function simulationHVACLoopRefGraphKey(loopType = "", loopName = "") {
  return `${simulationHVACGraphName(loopType)}:${simulationHVACGraphName(loopName)}`;
}
...
export function handleSimulationEnergyDashboardChange(event) {
  ...
  if (target.matches("[data-energy-path-chart-frequency]")) {
    if (target.disabled || !["monthly", "hourly"].includes(target.value) ||
      target.value === (state.simulationEnergyChartFrequency || "monthly")) return;
    state.simulationEnergyChartFrequency = target.value;
    updateSimulationEnergyPanel("chart");
    elements.simulationEnergyDashboard.querySelector("[data-energy-path-chart-frequency]")?.focus({ preventScroll: true });
    return;
  }
...
function handleSimulationHVACResultsInput(event) {
  if (!(event.target instanceof Element)) return;
  const picker = event.target.closest("[data-simulation-hvac-loop]");
  const { context, loop, ui } = simulationHVACInspectionContext();
  if (picker && event.type !== "click") {
    if (picker.value !== context.selectedLoop) {
      recordViewHistory();
      context.selectedLoop = picker.value;
      renderSimulationHVACLoops(state.simulationResult);
      elements.simulationHVACLoopResults.querySelector("[data-simulation-hvac-loop]")?.focus({ preventScroll: true });
    }
    return;
  }
...
function selectSimulationSeries(series, range = { start: 0, end: -1 }) {
  ...
  if (simulationSeriesPanels[0]) {
    // Different variables can have different timestamp grids. A contextual
    // jump owns this panel; retained overlays must not distort its month range.
    simulationSeriesPanels[0].seriesIDs = [state.simulationSelectedSeries];
    simulationSeriesPanels[0].start = range.start;
    simulationSeriesPanels[0].end = range.end;
  } else {
    simulationSeriesPanels = [{ id: simulationSeriesPanelSequence++, seriesIDs: [state.simulationSelectedSeries], ...range }];
  }
  state.simulationSeriesRangeStart = range.start;
  state.simulationSeriesRangeEnd = range.end;
...
function renderSimulationSeriesSelect(result) {
  ...
  if (!state.simulationSelectedSeries || !series.some((item) => seriesID(item) === state.simulationSelectedSeries)) {
    const migrated = findSimulationSeriesByID(state.simulationSelectedSeries);
    if (migrated) state.simulationSelectedSeries = seriesID(migrated);
    else if (!state.simulationSelectedSeries) state.simulationSelectedSeries = seriesID(preferredSimulationSeries(series) || series[0]);
  }
  ...
  if (!findSimulationSeriesByID(state.simulationSelectedSeries)) {
    simulationSeriesPanels[0].seriesIDs = [state.simulationSelectedSeries];
    simulationSeriesPanels[0].start = 0;
    simulationSeriesPanels[0].end = -1;
  }
  const validIDs = new Set(series.map(seriesID));
  simulationSeriesPanels.forEach((panel) => {
    panel.seriesIDs = panel.seriesIDs.map((id) => validIDs.has(id) ? id : energyPathSeriesID(findSimulationSeriesByID(id) || {}))
...
function renderSimulationSeriesPanel(panel, allSeries) {
  const selected = panel.seriesIDs.map((id) => findSimulationSeriesByID(id)).filter(Boolean);
...
function simulationSeriesPickerOptions(allSeries) {
  return (selectedID) => `${findSimulationSeriesByID(selectedID) ? "" : `<option value="" disabled selected>${escapeHTML(t("simulation.energyPathChooseSeries", {}, "Choose source series"))}</option>`}` + simulationSeriesGroupOptions(allSeries).filter((group) => group.value !== "all").map((group) => {
    const items = allSeries.filter((item) => simulationSeriesGroupID(item) === group.value);
    return items.length ? `<optgroup label="${escapeHTML(group.label)}">${items.map((item) => `<option value="${escapeHTML(seriesID(item))}" ${seriesID(item) === selectedID ? "selected" : ""}>${escapeHTML([item.column, item.reportingFrequency].filter(Boolean).join(" · "))}</option>`).join("")}</optgroup>` : "";
  }).join("");
}
...
function renderSimulationMultiSeriesSVG(seriesList, panel) {
  ...
  const unitRanges = new Map(units.map((unit) => {
    const values = seriesList.filter((s) => unitKey(s) === unit).flatMap((s) => simulationSeriesPoints(s).slice(panel.start, panel.end + 1).map((p) => Number(p.value))).filter(Number.isFinite);
    ...
    return [unit, { min, max, span: max - min }];
  }));
  const lines = seriesList.map((series, index) => {
    const points = simulationSeriesPoints(series).slice(panel.start, panel.end + 1), scale = unitRanges.get(unitKey(series));
    ...
    const dash = ["", "8 4", "3 3", "10 3 2 3"][index % 4];
  ...
  const xTicks = xTickIndexes.map((pointIndex) => {
    const x = pad.left + (referencePoints.length > 1 ? pointIndex * plotWidth / (referencePoints.length - 1) : plotWidth / 2);
...
function bindSimulationChartInteractions() {
  ...
  host.addEventListener(
    ...
    (event) => {
      ...
      if (!hitTarget) return;
      const panel = simulationSeriesPanels.find((item) => String(item.id) === hitTarget.dataset.seriesPanelId);
      const series = panel?.seriesIDs.map((id) => simulationSeriesLookup().byID.get(id)).find(Boolean);
      if (!panel || !series) return;
      event.preventDefault();
      zoomSimulationSeriesPanel(event, panel, series, hitTarget);
    },
    { passive: false },
  );
  host.addEventListener("dblclick", (event) => {
    const hit = event.target instanceof Element ? event.target.closest("[data-simulation-chart-hit]") : null;
    const panel = hit && simulationSeriesPanels.find((item) => String(item.id) === hit.dataset.seriesPanelId);
    if (!panel) return;
    panel.start = 0; panel.end = -1;
    renderSimulationChart();
  });
  host.addEventListener("pointerdown", (event) => {
    ...
    simulationSeriesPan = { pointerId: event.pointerId, panel, startX: event.clientX, start: panel.start, end: panel.end, width: hit.getBoundingClientRect().width };
    hit.setPointerCapture(event.pointerId);
  });
  host.addEventListener("pointermove", (event) => {
    if (!simulationSeriesPan || simulationSeriesPan.pointerId !== event.pointerId) return;
    const { panel, startX, start, end, width } = simulationSeriesPan;
    const size = end - start + 1;
    const shift = Math.round((startX - event.clientX) / Math.max(width, 1) * size);
    const first = simulationSeriesLookup().byID.get(panel.seriesIDs[0]);
    const max = Math.max(0, simulationSeriesPointCount(first) - 1);
    panel.start = Math.max(0, Math.min(max - size + 1, start + shift));
    panel.end = Math.min(max, panel.start + size - 1);
  });
  const stopPan = (event) => {
    if (simulationSeriesPan?.pointerId !== event.pointerId) return;
    simulationSeriesPan = null;
    renderSimulationChart();
  };
...
function zoomSimulationSeriesPanel(event, panel, series, hitTarget = event.currentTarget) {
  const pointCount = simulationSeriesPointCount(series);
  if (pointCount <= 2) return;
  const start = panel.start || 0, end = panel.end < 0 ? pointCount - 1 : panel.end;
  ...
  if (nextSize >= pointCount) {
    panel.start = 0; panel.end = pointCount - 1;
    renderSimulationChart();
    return;
  }
...
function seriesID(series) {
  return energyPathSeriesID(safeSimulationSeries(series));
}
