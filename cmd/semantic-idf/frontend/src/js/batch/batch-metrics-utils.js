const METRIC_NUMBER_PATTERN = /^[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?/;

export function parseMetricNumber(value) {
  const text = String(value ?? "").trim();
  const token = text.match(METRIC_NUMBER_PATTERN)?.[0] || "";
  if (!token) {
...
export function metricUnit(metric, displayValue) {
  const unit = String(metric?.unit || "").trim();
  if (unit) {
    return unit;
  }
  const text = String(displayValue ?? "").trim();
  const number = parseMetricNumber(text);
