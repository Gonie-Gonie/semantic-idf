...
func TestEPATH161ActualAppSelectionWithoutGraphRedrawBrowser(t *testing.T) {
	...
	modules := map[string]string{
		...
		"views/simulation-views.js":           profile("views/simulation-views.js", "simulationEnergyInspectorActions", "simulationEnergyDriverNavigation", "simulationEnergyServiceNavigation", "prepareSimulationEnergyModelNavigation", "simulationSemanticNavigationCache", "simulationEnergyGraphFocusTarget"),
...
try{
 if(innerWidth!==1600||innerHeight!==900){await report();}
  ...
  const graphSelector="[data-energy-path-canvas],[data-energy-path-layout-node],[data-energy-path-bar],[data-energy-path-ribbon],[data-energy-path-bridge-ratio],[data-energy-path-ratio-tooltip],[data-energy-path-link-hit],[data-energy-path-hit-layer],.energy-path-controls,[data-energy-path-quality-line]";
  ...
  const assertRetained=(before,label)=>{const current=retain();check(current.length===before.length&&current.every((item,index)=>item.element===before[index].element&&item.geometry===before[index].geometry&&JSON.stringify(item.rect)===JSON.stringify(before[index].rect)),label+" replaced or moved graph/header/quality DOM");};
  ...
  const measure=(label,activate,expectedID,{link=false,keepInspector=false}={})=>{
   const retained=retain(),before=counters(),oldInspector=host.querySelector("[data-energy-path-component-chart]"),details=host.querySelector("[data-energy-path-data-details]");
   ...
   const inspector=host.querySelector("[data-energy-path-component-chart]");
   ...
   if(expectedID){check(state.simulationEnergySelection===expectedID&&inspector?.getAttribute("data-energy-path-component-chart")===expectedID,label+" did not synchronously update selected inspector");check(rect?.width>0&&rect.height>0&&style.display!=="none"&&style.visibility!=="hidden",label+" measured a deferred/hidden inspector");check(inspector.querySelector("[data-energy-path-chart-frequency]")&&!inspector.querySelector("[data-energy-path-detail-section]"),label+" did not synchronously render chart controls");}
   if(keepInspector)check(inspector===oldInspector,label+" replaced unchanged current inspector");
   assertRetained(retained,label);const selectionWork=changed(before,counters());if(!chartCanvases.has(canvas)&&selectionWork.energyPathGraphForState===12)delete selectionWork.energyPathGraphForState;chartCanvases.add(canvas);check(Object.keys(selectionWork).length===0,label+" recomputed scene/projection/layout beyond initial monthly chart preparation: "+JSON.stringify(selectionWork));if(details&&expectedID)check(host.querySelector("[data-energy-path-data-details]")===details,label+" replaced independent Data drawer");
   return inspector;
  };
  ...
  if(new URLSearchParams(location.search).get("run161")!=="1"){document.body.dataset.epath161Status="manual";}
   ...
   activateNode(cooling,"visible cooling node");
   const inspector=host.querySelector("[data-energy-path-component-chart]");
   const beforeDetails=counters(),retained=retain();host.querySelector('[data-energy-path-quality-stage="drivers"]').click();check(host.querySelector("[data-energy-path-component-chart]")===inspector,"drawer open replaced inspector");assertRetained(retained,"Data drawer open");assertNoWork(beforeDetails,"Data drawer open");
   for(const tab of["output","data"]){const before=counters(),scene=retain();host.querySelector('[data-energy-path-details-tab="'+tab+'"]').click();check(host.querySelector("[data-energy-path-component-chart]")===inspector,"drawer tab replaced inspector");assertRetained(scene,"drawer "+tab);assertNoWork(before,"drawer "+tab);}
   ...
   invalidate("period",()=>change("[data-simulation-energy-path-period]","M1"));
   ...
   evidence.push("Graph/header/quality/ribbon/hit/ratio DOM retained; after initial monthly chart preparation, selections and drawer actions do no projection/layout work; context/result invalidation rebuilt geometry");await report();
  }
 }
