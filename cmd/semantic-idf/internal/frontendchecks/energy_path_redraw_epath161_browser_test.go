...
func TestEPATH161ActualAppSelectionWithoutGraphRedrawBrowser(t *testing.T) {
	...
	profile := func(path string, functions ...string) string {
		...
		for _, name := range functions {
			...
			source = strings.Replace(source, marker, "function "+original+"(", 1)
			source += fmt.Sprintf("\n%sfunction %s(...args) { const started = performance.now(); try { return %s(...args); } finally { const timings = globalThis.epath161InspectorTimings ||= {}; timings[%q] = (timings[%q] || 0) + performance.now() - started; } }\n", export, name, original, name, name)
		}
		return source
	}
	modules := map[string]string{
		...
		"views/simulation-views.js":           profile("views/simulation-views.js", "simulationEnergyInspectorActions", "simulationEnergyInspectorRelatedEntities", "simulationEnergyDriverNavigation", "simulationEnergyServiceNavigation"),
		"energy-path-driver-destinations.js":  profile("energy-path-driver-destinations.js", "energyPathDriverDestinations"),
...
try{
 if(innerWidth!==1600||innerHeight!==900){await report();}
  ...
  if(new URLSearchParams(location.search).get("run161")!=="1"){document.body.dataset.epath161Status="manual";}
   ...
   const people=initialGraph.nodes.find(item=>item.driverCategory==="internal.people");activateNode(people,"metadata refresh setup");const previousInspector=host.querySelector("[data-energy-path-inspector]");state.report={...state.report};activateNode(people,"same selection fresh report metadata");check(host.querySelector("[data-energy-path-inspector]")!==previousInspector,"fresh report did not refresh same-node inspector");
