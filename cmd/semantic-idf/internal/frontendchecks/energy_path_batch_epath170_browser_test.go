...
try{
 ...
 if(new URLSearchParams(location.search).get("manual")==="1"){
  ...
  check(!detail.querySelector(".energy-path-kpis,[data-energy-path-kpi-node]"),"Batch detail still renders redundant summary cards");
  node.click();const frequency=detail.querySelector("[data-energy-path-chart-frequency]");check(frequency?.value==="monthly","Batch component chart has no Monthly default");
  if(frequency){frequency.value="hourly";frequency.dispatchEvent(new Event("change",{bubbles:true}));check(detail.querySelector("[data-energy-path-chart-frequency]")?.value==="hourly"&&detail.querySelector("[data-energy-path-canvas]")===canvas,"Batch hourly selection rebuilt graph or did not update chart");}
