...
func TestThermalTopologyLayoutBrowserHarness(t *testing.T) {
	...
	for _, signal := range []string{`"deterministic":true`, `"noOverlap":true`, `"portsAvoidCenters":true`, `"directionalPlacement":true`, `"parallelOffset":true`, `"selectionCacheStable":true`, `"levelCacheChanges":true`, `"allLevelZones":3`, `"filteredLevelZones":2`, `"focusOneHop":true`, `"boundaryFocus":true`, `"sharedZoneFocus":true`, `"sharedSpaceFocus":true`, `"sharedBoundaryFocus":true`, `"adiabaticStub":true`, `"noCommonAdiabaticNode":true`} {
...
func TestThermalTopologyRendererBrowserHarness(t *testing.T) {
	...
	for _, signal := range []string{`"svg":true`, `"metricWidth":true`, `"inspector":true`, `"accessibleTargets":true`, `"deterministicTabOrder":true`, `"keyboardNode":true`, `"oneHopFocus":true`, `"inspectorHeading":true`, `"patternLegend":true`, `"zoneOnly":true`, `"grossOnly":true`} {
		if !strings.Contains(document, signal) {
...
try {
  ...
  const focus = await import("/src/js/topology-focus.js");
  ...
  const options = { layout: "spatial", storyIndex: "all", showAirCoupling: true };
  ...
  const levelCacheChanges = layout.thermalTopologyLayoutCacheKey(geometry, options, {width:900,height:600}) !== layout.thermalTopologyLayoutCacheKey(geometry, {...options,storyIndex:0}, {width:900,height:600});
  assert(selectionCacheStable && levelCacheChanges, "selection/level layout cache boundary is incorrect");
  const allLevelZones = model.nodes.filter((node) => node.kind === "zone").length;
  const levelModel = layout.createThermalTopologyLayoutModel(geometry, { ...options, storyIndex:0 });
  const filteredLevelZones = levelModel.nodes.filter((node) => node.kind === "zone").length;
  assert(allLevelZones === 3, "Level All did not retain every zone");
  assert(filteredLevelZones === 2 && !levelModel.nodes.some((node) => node.id === "zone:c"), "specific Level leaked a zone from another story");
  const zoneFocus = layout.thermalTopologyFocusContext(model, "zone:a");
  const focusOneHop = zoneFocus.active && zoneFocus.nodeIDs.has("zone:a") && zoneFocus.nodeIDs.has("zone:b") && !zoneFocus.nodeIDs.has("zone:c") && zoneFocus.edgeIDs.has("edge:ab") && !zoneFocus.edgeIDs.has("edge:bc");
  assert(focusOneHop, "zone focus did not retain exactly one-hop related objects");
  const boundaryFocusContext = layout.thermalTopologyFocusContext(model, "boundary:adiabatic");
  const boundaryFocus = boundaryFocusContext.active && boundaryFocusContext.nodeIDs.has("zone:b") && !boundaryFocusContext.nodeIDs.has("zone:c");
  assert(boundaryFocus, "boundary focus did not isolate its owning zone");
  const focusGeometry = {topology:{
    nodes:[
      {id:"zone:a",kind:"zone"},{id:"space:a",kind:"space"},{id:"zone:b",kind:"zone"},{id:"zone:c",kind:"zone"},
    ],
    connections:[
      {id:"connection:ab",fromNodeId:"zone:a",toNodeId:"zone:b",boundaryIds:["boundary:a","boundary:b"]},
      {id:"connection:bc",fromNodeId:"zone:b",toNodeId:"zone:c",boundaryIds:["boundary:b2","boundary:c"]},
    ],
    boundaries:[
      {id:"boundary:a",surfaceId:"surface:a",ownerZoneId:"zone:a",targetKind:"zone",targetId:"zone:b",counterpartSurfaceId:"surface:b"},
      {id:"boundary:b",surfaceId:"surface:b",ownerZoneId:"zone:b",targetKind:"zone",targetId:"zone:a",counterpartSurfaceId:"surface:a"},
      {id:"boundary:b2",surfaceId:"surface:b2",ownerZoneId:"zone:b",targetKind:"zone",targetId:"zone:c",counterpartSurfaceId:"surface:c"},
      {id:"boundary:c",surfaceId:"surface:c",ownerZoneId:"zone:c",targetKind:"zone",targetId:"zone:b",counterpartSurfaceId:"surface:b2"},
    ], openings:[], airCouplings:[], issueLinks:[], zoneSignatures:[
      {zoneId:"zone:a",spaceIds:["space:a"]},
    ],
  }};
  const sharedZone = focus.createTopologyFocusContext(focusGeometry,{kind:"zone",id:"zone:a"});
  const sharedZoneFocus = sharedZone.active && sharedZone.zoneIds.has("zone:a") && sharedZone.zoneIds.has("zone:b") && !sharedZone.zoneIds.has("zone:c") && sharedZone.connectionIds.has("connection:ab") && !sharedZone.connectionIds.has("connection:bc");
  assert(sharedZoneFocus, "shared focus resolver did not stop at the selected zone's one-hop neighbor");
  const sharedSpace = focus.createTopologyFocusContext(focusGeometry,{kind:"space",id:"space:a"});
  const sharedSpaceFocus = sharedSpace.active && sharedSpace.nodeIds.has("space:a") && sharedSpace.zoneIds.has("zone:a") && sharedSpace.zoneIds.has("zone:b") && !sharedSpace.zoneIds.has("zone:c") && sharedSpace.connectionIds.has("connection:ab") && !sharedSpace.connectionIds.has("connection:bc");
  assert(sharedSpaceFocus, "shared focus resolver did not follow a compacted Space endpoint through its parent zone");
  const sharedBoundary = focus.createTopologyFocusContext(focusGeometry,{kind:"surface",id:"surface:a"});
  const sharedBoundaryFocus = sharedBoundary.active && sharedBoundary.zoneIds.has("zone:a") && sharedBoundary.zoneIds.has("zone:b") && !sharedBoundary.zoneIds.has("zone:c") && sharedBoundary.surfaceIds.has("surface:a") && sharedBoundary.surfaceIds.has("surface:b") && sharedBoundary.connectionIds.has("connection:ab");
  assert(sharedBoundaryFocus, "shared focus resolver did not retain both zones and surfaces of a boundary");
  ...
  document.getElementById("result").textContent = JSON.stringify({ deterministic, noOverlap, portsAvoidCenters, directionalPlacement, parallelOffset, selectionCacheStable, levelCacheChanges, allLevelZones, filteredLevelZones, focusOneHop, boundaryFocus, sharedZoneFocus, sharedSpaceFocus, sharedBoundaryFocus, adiabaticStub, noCommonAdiabaticNode });
...
try {
  ...
  const geometry = { topology: {
    ...
    nodes: [
      ...
      { id: "zone:c", entityId: "zone:c", kind: "zone", label: "Zone C", storyIndex: 0, centroid: {x:16,y:0,z:0} },
  ...
  assert(svg && metricWidth && inspector && accessibleTargets && deterministicTabOrder && keyboardNode && oneHopFocus && inspectorHeading && patternLegend && zoneOnly && grossOnly, "renderer contract failed");
  document.getElementById("result").textContent = JSON.stringify({svg,metricWidth,inspector,accessibleTargets,deterministicTabOrder,keyboardNode,oneHopFocus,inspectorHeading,patternLegend,zoneOnly,grossOnly});
...
try {
  ...
  const state = stateModule.state;
  state.report={geometry}; state.topologyMode="thermal"; state.selectedTopologyStory="all"; state.thermalTopologyMetric="topology"; state.thermalTopologyLayout="network";
  state.thermalTopologyPanX=30; state.thermalTopologyPanY=-20; state.thermalTopologyScale=2; state.thermalTopologyLayoutCache.clear();
