...
func parseSimulationEnergyExplanationCanonicalSQL(path string, plan *PurposeRunPlan, driverContext energyDriverBuildContext) (energyExplanationParseResult, error) {
	...
	centralHeatPumpPools, centralHeatPumpSources := readEnergyPathCentralHeatPumpMonthlyPools(db, filepath.Base(path), plan, driverContext, series)
	consumptionPools = append(consumptionPools, centralHeatPumpPools...)
	consumptionSources = mergeEnergyPathPoolSources(consumptionSources, centralHeatPumpSources)
